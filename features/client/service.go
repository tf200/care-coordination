...
func (s *clientService) MoveClientToWaitingList(
	ctx context.Context,
	...
	if err != nil {
		s.logger.Error(
			ctx,
			"MoveClientToWaitingList",
			"Failed to create client and update intake form",
			zap.Error(err),
		)
...
func (s *clientService) MoveClientInCare(
	ctx context.Context,
	clientID string,
	...
	if client.Status != db.ClientStatusEnumWaitingList {
		s.logger.Error(
			ctx,
			"MoveClientInCare",
			"Client must be on waiting list to move to in care",
			zap.String("currentStatus", string(client.Status)),
		)
	...
	if isAmbulatory && (req.AmbulatoryWeeklyHours == nil || *req.AmbulatoryWeeklyHours <= 0) {
		s.logger.Error(
			ctx,
			"MoveClientInCare",
			"Ambulatory weekly hours required for ambulatory care",
		)
	...
	if !isAmbulatory && req.AmbulatoryWeeklyHours != nil {
		s.logger.Error(
			ctx,
			"MoveClientInCare",
			"Ambulatory weekly hours should only be set for ambulatory care",
			zap.String("careType", string(client.CareType)),
		)
	...
	s.logger.Info(
		ctx,
		"MoveClientInCare",
		"Client moved to in care successfully",
		zap.String("clientId", updatedClient),
	)
...
func (s *clientService) StartDischarge(
	ctx context.Context,
	clientID string,
	...
	if client.Status != db.ClientStatusEnumInCare {
		s.logger.Error(
			ctx,
			"StartDischarge",
			"Client must be in care to start discharge",
			zap.String("currentStatus", string(client.Status)),
		)
	...
	if client.DischargeStatus.Valid {
		s.logger.Error(
			ctx,
			"StartDischarge",
			"Discharge already started for this client",
			zap.String("dischargeStatus", string(client.DischargeStatus.DischargeStatusEnum)),
		)
	...
	updateParams := db.UpdateClientParams{
		...
		Status: db.NullClientStatusEnum{
			ClientStatusEnum: db.ClientStatusEnumInCare,
			Valid:            true,
		}, // Client remains in care during phase 1
		DischargeDate: util.StrToPgtypeDate(req.DischargeDate),
		ReasonForDischarge: db.NullDischargeReasonEnum{
			DischargeReasonEnum: db.DischargeReasonEnum(req.ReasonForDischarge),
			Valid:               true,
		},
	...
	s.logger.Info(
		ctx,
		"StartDischarge",
		"Discharge process started for client",
		zap.String("clientId", updatedClient),
	)
...
func (s *clientService) CompleteDischarge(
	ctx context.Context,
	clientID string,
	...
	if client.Status != db.ClientStatusEnumInCare {
		s.logger.Error(
			ctx,
			"CompleteDischarge",
			"Client must be in care to complete discharge",
			zap.String("currentStatus", string(client.Status)),
		)
		return nil, ErrClientNotInCare
	}
	...
	if !client.DischargeStatus.Valid ||
		client.DischargeStatus.DischargeStatusEnum != db.DischargeStatusEnumInProgress {
...
func (s *clientService) ListWaitingListClients(
	ctx context.Context,
...
func (s *clientService) ListInCareClients(
	ctx context.Context,
...
func (s *clientService) ListDischargedClients(
	ctx context.Context,
